    ...
    open func drawLinear(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        { // only one color per dataset
            ...
            if e1 != nil
            {
                ...
                if !firstPoint
                {
                    //check here. change color of lines tapped on graph
                    context.setStrokeColor(dataSet.color(atIndex: 0).cgColor)
                    print(dataSet.color(atIndex: 0).cgColor)
...
//            let color = NSUIColor(red: 203/255.0, green: 247/255.0, blue: 240/255.0, alpha: 1.0)

            //check here. method call after tap on graph
